...
def expense_tracker_gui(user_id):
    ...
    window.style.set_theme('radiance')
...
def filter_transactions_ui(tab, user_id):
    ...
    # Date Range Filter
    tk.Label(tab, text="Date Range:").pack()
    ...
    def filter_transactions():
        ...
        cursor.execute('''SELECT * FROM transactions WHERE user_id = ? AND category = ? 
                          AND date BETWEEN ? AND ?''', (user_id, category, start_date, end_date))
        transactions = cursor.fetchall()
        conn.close()

        for trans in transactions:
            print(trans)  # This is where you would update the UI to display filtered transactions.

    tk.Button(tab, text="Filter", command=filter_transactions).pack(pady=10)
...
def view_analytics_ui(tab, user_id):
    ...
    def show_budget_analytics():
        ...
        if budget_result:
            ...
            used_percentage = (total_expenses / budget) * 100 if budget else 0

            # Show analytics (e.g., with Matplotlib)
            categories = ['Expenses', 'Remaining']
            data = [total_expenses, remaining_budget]
            plt.pie(data, labels=categories, autopct='%1.1f%%', startangle=140)
            plt.title('Budget Usage')
            plt.show()
        else:
            messagebox.showinfo("No Budget Set", "Please set a budget first.")

    tk.Button(tab, text="Show Analytics", command=show_budget_analytics).pack(pady=10)
